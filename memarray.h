...
/** 内存页地址信息双向链表 */
typedef struct _mem_array_node_t {
    struct _mem_array_node_t    *prev, *next;
    uint8_t                     *data;
} _mem_array_node_t;

typedef struct _mem_array_head_t {
    _mem_array_node_t    *prev, *next;
} _mem_array_head_t;
...
typedef struct mem_array_t {
    ...
    _mem_array_head_t list_head;// 内存页链表
} mem_array_t;
...
// 设置自定义的内存分配释放函数，缺省使用malloc和free
 ...
 * @return          返回缓冲区对象
*/
extern mem_array_t *mem_array_malloc(uint32_t pagesize);
