...
bool aes_setkey_enc(aes_ctx_t* pctx, const unsigned char *key, unsigned int keysize) {
    pctx->mode = AES_ENCRYPT;
    unsigned int i, imax;
    uint32_t *RK;

    switch(keysize) {
        case 128: pctx->nr = 10; break;
        case 192: pctx->nr = 12; break;
        case 256: pctx->nr = 14; break;
        default : return false;
    }

    pctx->rk = RK = pctx->buf;

    for(i = 0, imax = keysize >> 5; i < imax; i++)
...
bool aes_setkey_dec(aes_ctx_t* pctx, const unsigned char *key, unsigned int keysize) {
    ...
    bool ret = aes_setkey_enc( &cty, key, keysize );
    if(!ret) return ret;
    ...
    *RK++ = *SK++;
    ...
    for(i = pctx->nr - 1, SK -= 8; i > 0; i--, SK -= 8) {
        for(j = 0; j < 4; j++, SK++) {
            *RK++ = RT0[ FSb[ ( *SK       ) & 0xFF ] ] ^
                    RT1[ FSb[ ( *SK >>  8 ) & 0xFF ] ] ^
                    RT2[ FSb[ ( *SK >> 16 ) & 0xFF ] ] ^
...
void aes_crypt_ecb(aes_ctx_t* pctx, const unsigned char input[16], unsigned char output[16]) {
    ...
    if(pctx->mode == AES_DECRYPT) {
        for(i = (pctx->nr >> 1) - 1; i > 0; i--) {
...
bool aes_crypt_cbc(aes_ctx_t* pctx, size_t length, unsigned char iv[16], const unsigned char *input, unsigned char *output) {
    ...
    if(pctx->mode == AES_DECRYPT) {
        while(length > 0) {
            ...
            aes_crypt_ecb(pctx, input, output);
        ...
        while(length > 0) {
            ...
            aes_crypt_ecb(pctx, output, output);
            memcpy(iv, output, 16);
...
void aes_init(aes_ctx_t* pctx, int mode, const unsigned char *key, uint32_t keysize, const unsigned char iv[16]) {
    pctx->mode = mode;
    memcpy(pctx->iv, iv, 16);
    if (mode == AES_ENCRYPT)
        aes_setkey_enc(pctx, key, keysize);
    else
        aes_setkey_dec(pctx, key, keysize);
    pctx->input_len = 0;
}
...
size_t aes_update(aes_ctx_t* pctx, const void* input, size_t input_len, void* output) {
    ...
    if (pl) { // 缓存有内容，优先处理缓存
        bool is_full = pl + input_len >= 16; // 本次加密内容加上原有的缓存是否能够满足至少8个字节长度
        uint32_t copy_len = is_full ? 16 - pl : input_len;
        memcpy(pctx->input + pl, ip, copy_len); // 复制到缓存
        if (!is_full) { // 缓存未满8字节，无需加密
            pctx->input_len += copy_len;
            return 0;
        }
        aes_crypt_cbc(pctx, 16, pctx->iv, pctx->input, op);
        input_len -= copy_len;
        ip += copy_len;
        pctx->input_len = 0;
        op += 16;
    ...
    uint32_t block_size = (input_len >> 4) << 4;
    if (block_size) {
        aes_crypt_cbc(pctx, block_size, pctx->iv, ip, op);
        ip += block_size;
        op += block_size;
...
size_t aes_final(aes_ctx_t* pctx, const void* input, size_t input_len, void* output) {
    ...
    op += aes_update(pctx, input, input_len, op);
    if (pctx->mode == AES_ENCRYPT) {
        aes_pkcs7_padding(pctx->input, pctx->input_len);
        aes_crypt_cbc(pctx, 16, pctx->iv, pctx->input, op);
        op += 16;
    } else {
        op -= *(op - 1);
...
int main() {
    ...
    size_t len = PKCS7_PADDING_SIZE(tlen, 16);
    unsigned char data[len], *pdata = data;

    aes_ctx_t ctx;
    aes_init(&ctx, AES_ENCRYPT, pwd, 128, iv);
    pdata += aes_update(&ctx, text, 7, pdata);
    pdata += aes_update(&ctx, text + 7, 2, pdata);
    pdata += aes_final(&ctx, text + 9, tlen - 9, pdata);
    if (len != pdata - data || memcmp(data, enc_data, len)) {
        printf("aes enc fail!\n");
        for (size_t i = 0; i < len; i++)
            printf("%02x", data[i]);
        printf("\n");
    } else
        printf("aes enc succeed!\n");
    
    pdata = data;
    aes_init(&ctx, AES_DECRYPT, pwd, 128, iv);
    size_t dlen = aes_final(&ctx, data, len, data);
    if (dlen != tlen || memcmp(data, text, tlen)) {
        printf("aes dec fail!\n");
    } else {
        printf("aes dec succeed!\n");
    }
}
#endif